...
- Локальная личность использует временный placeholder для ключевого материала.
- QR-сопряжение содержит placeholder-доказательства, а не промышленные подписи.
- Пакет `KrakenPacket` содержит `proofMode = prototype-placeholder`.
- Legacy payload сообщений ещё может оставаться prototype/plain JSON только в
...
Это и есть вклад алгоритма в production crypto-path Kraken: он не заменяет AEAD,
но становится обязательной политикой допуска и контекстной привязкой для
криптографического envelope. Payload может быть открыт только в том же
relationship/session/profile context, который прошёл локальную проверку
диагностики кручения.
...
7. Отдельным этапом завершить промышленные примитивы:
   ...
   - разделение debug/prototype и release/prod build flavors;
...
- “Рациональная диагностика над `Q` доказывает стойкость схем над конечными
  полями”.
- “`PrototypeNoSecurityPacketCrypto` является подписью или шифрованием”.
...
- Unit-тесты production-policy, которая запрещает plaintext outbox/inbox без
...
- Отдельный отчёт, что Adamova-bound encrypted message path уже реализован, но
  полный production messenger crypto всё ещё требует key agreement/KDF,
  Keystore, подписей, canonical envelope, replay protection и внешнего review.
