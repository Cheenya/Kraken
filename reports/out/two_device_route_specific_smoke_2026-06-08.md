...
The earlier fresh LAN Bluetooth-off run is retained as regression evidence: Device A -> Device B failed with `ECONNREFUSED` to stale Device B port `42817`, while Device B was actually listening on a different port in `ss`. The current fix removes stale endpoints by peer fingerprint/service name and retries fresh LAN candidates.
...
- BLE retry-delivered export time: Device A `2026-06-08 01:33:56 MSK`, Device B `2026-06-08 01:33:56 MSK`
...
- LAN stale-peer-fix export time: Device A `2026-06-08 01:44:45 MSK`, Device B `2026-06-08 01:44:47 MSK`
...
| Role | Serial | Device |
| --- | --- | --- |
| Device A | `ANDROID_DEVICE_A` | `device_a ANDROID_MODEL_A` |
| Device B | `ANDROID_DEVICE_B` | `Device B ANDROID_MODEL_B` |

## Scenario Status

| Scenario | Status | Evidence |
| --- | --- | --- |
| Device A -> Device B over LAN NSD/TCP | Passed current LAN after stale-peer fix | Bluetooth-off run after fix: Device A selected `lan-nsd-tcp`, `sent=true`, `delivered=true`, status `DELIVERED_TO_PEER`, latency `1391 ms` |
| Device B -> Device A over LAN NSD/TCP | Passed current LAN after stale-peer fix | Bluetooth-off run after fix: Device B selected `lan-nsd-tcp`, recovered after stale-port refusal, then `sent=true`, `delivered=true`, status `DELIVERED_TO_PEER`, latency `929 ms` |
| Device A -> Device B over BLE direct | Passed current route evidence + older physical UI | Current retry run observed `ble-gatt` and delivered on both exports; older physical UI had `bleS2X2101`, `bleFinal2105` delivered |
| Unknown peer rejection | Passed current debug-only counter evidence | BLE retry-delivered export: Device A `2`, Device B `2` |
| Wrong recipient rejection | Passed current debug-only counter evidence | BLE retry-delivered export: Device A `1`, Device B `1` |
| Duplicate packet rejection | Passed current debug-only counter evidence | BLE retry-delivered export: Device A `1`, Device B `1` |
...
- recent route results include one stale Device A endpoint refusal, then fresh `lan-nsd-tcp:true` attempts.

## Pre-Fix LAN Regression Evidence

- Device A -> Device B before stale-peer cleanup: `lan-nsd-tcp:false`, `ECONNREFUSED` to Device B `192.168.0.121:42817`, while Device B was listening on another port.
- Device B -> Device A before stale-peer cleanup: `lan-nsd-tcp:true`, `sent=true`, `delivered=false`.
