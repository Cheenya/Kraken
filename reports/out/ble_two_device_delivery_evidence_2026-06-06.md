...
Status: manual Device A/Device B BLE direct-route evidence captured.
...
| --- | --- | --- |
| Device A | Device A `ANDROID_DEVICE_A` | `after-open/device-a-ANDROID_DEVICE_A/screen.png`, `after-open/device-a-ANDROID_DEVICE_A/window.xml`, `device-a-ANDROID_DEVICE_A/bluetooth-manager.txt` |
| Device B | Device B `ANDROID_DEVICE_B` | `device-b-ANDROID_DEVICE_B/bluetooth-manager.txt`, `after-open/device-b-ANDROID_DEVICE_B/screen.png`, `after-open/device-b-ANDROID_DEVICE_B/window.xml` |

## App-Level Evidence

Device A `after-open/device-a-ANDROID_DEVICE_A/window.xml` shows an open chat with
`Device B` and the route label:
...
```text
artifacts/two-phone-test/ble-live-20260606-204431/after-open/device-a-ANDROID_DEVICE_A/screen.png
```

This is direct UI evidence that the route-aware layer selected BLE for the
Device A -> Device B chat at capture time. It is stronger than merely proving that
...
> chat messages between QR-trusted Device A/Device B devices.
