...
Единая формулировка бренда для всех платформ: `ПРИВАТНО  •  ЛОКАЛЬНО  •  СВОБОДНО`.
...
`app-ios/` - нативная iOS-сборка для Simulator и проверки мобильного интерфейса.
...
проверочных файлов внутри Android-сборки такой:
