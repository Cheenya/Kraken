...
def mode_result(mode: RelayAttackMode) -> dict[str, Any]:
    ...
    decision = relay.handle_packet(packet(f"packet-{mode.value}"), source_host="192.168.1.10")
    return {
        "mode": mode.value,
...
def main() -> int:
    ...
    report = {
        ...
        "claim_boundary": "desktop_lan_relay_decision_check",
    ...
    for item in report["modes"]:
        lines.append(
            f"- `{item['mode']}`: принято=`{item['accepted']}`, причина=`{item['reason']}`, "
            f"передано=`{item['forwarded']}`, в очереди=`{item['queued']}`, отброшено=`{item['dropped']}`"
        )
    lines.extend(
        [
            "",
            "## Граница проверки",
            "",
            "Этот артефакт проверяет только локальную логику решения relay. Доставка по Android-радиоканалу подтверждается отдельным телефонным прогоном.",
        ]
    )
