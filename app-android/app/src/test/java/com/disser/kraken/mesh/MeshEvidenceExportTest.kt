...
class MeshEvidenceExportTest {
    ...
    fun routeSpecificEvidenceJsonContainsRouteCountersBuildAndDeviceShape() {
        ...
        assertTrue(markdown.contains("Выбранный маршрут: `ble-gatt`"))
        assertTrue(markdown.contains("Включённые режимы связи: `ble-gatt, lan-nsd-tcp`"))
        assertTrue(markdown.contains("Регистрация: `wifi-direct:registered:_kraken._tcp.`"))
        assertTrue(markdown.contains("Обнаружение: `wifi-direct:discovering:_kraken._tcp.`"))
        assertTrue(markdown.contains("Найдено устройств транспортом: `1`"))
        assertTrue(markdown.contains("Префиксы отпечатков найденных устройств: `peer-fingerp`"))
        assertTrue(markdown.contains("Видимые P2P-устройства: `2`"))
        assertTrue(markdown.contains("Статус этого P2P-устройства: `available`"))
        assertTrue(markdown.contains("Циклы обнаружения: `3`"))
        assertTrue(markdown.contains("Локальный endpoint транспорта: `192.168.1.10:49152`"))
        assertTrue(markdown.contains("P2P interface addresses: `192.168.49.1`"))
        assertTrue(markdown.contains("Группа Wi‑Fi Direct создана: `true`"))
        assertTrue(markdown.contains("Роль в группе Wi‑Fi Direct: `owner`"))
        assertTrue(markdown.contains("Локальный P2P-адрес Wi‑Fi Direct: `192.168.49.1`"))
        assertTrue(markdown.contains("Адрес bind сервера Wi‑Fi Direct: `192.168.49.1`"))
        assertTrue(markdown.contains("Последний endpoint отправки Wi‑Fi Direct: `192.168.49.59:43195`"))
        assertTrue(markdown.contains("Состояние endpoint binding Wi‑Fi Direct: `BOUND`"))
        assertTrue(markdown.contains("Причина endpoint binding Wi‑Fi Direct: `fallback-p2p-client-host`"))
        assertTrue(markdown.contains("Найденные peer endpoints Wi‑Fi Direct: `1`"))
        assertTrue(markdown.contains("Видимые устройства Wi‑Fi Direct: `1`"))
        assertTrue(markdown.contains("Диагностика TXT records Wi‑Fi Direct: `1`"))
        assertTrue(markdown.contains("Привязанные endpoints Wi‑Fi Direct: `1`"))
        assertTrue(markdown.contains("Кандидаты ретрансляции реалма: `1`"))
        assertTrue(markdown.contains("Префиксы кандидатов ретрансляции реалма: `relay-finge`"))
        assertTrue(markdown.contains("Wi‑Fi Direct fine location declared/granted/app-op: `true/false/ignored`"))
        assertTrue(markdown.contains("Предупреждение разрешений Wi‑Fi Direct: `wifi-direct-fine-location-missing-modern-android`"))
        ...
        assertTrue(markdown.contains("Режим проверки: `debug_local_inbox_packet_injection_and_queue_retry_probe`"))
        ...
        assertTrue(markdown.contains("Граница evidence: защищённый путь данных сообщения, проверка допуска профиля, диагностика транспорта и двухтелефонные проверки."))
    }
