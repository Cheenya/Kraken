...
object MeshEvidenceExporter {
    ...
    const val CLAIM_BOUNDARY =
        "p2p_integrity_and_adamova_admission_evidence_scope"
    const val INTEGRITY_MODE = "adamova-bound-aes-gcm-message-payload-context-binding"
    ...
    private const val ADAMOVA_REFERENCE_VALIDATION_STATUS = "local-native-diagnostic-backend"
    ...
    fun toMarkdownSummary(export: MeshEvidenceSnapshotExport): String =
        buildString {
            appendLine("# Сводка route-specific evidence Kraken")
            appendLine()
            appendLine("- Версия отчёта: `${export.reportVersion}`")
            appendLine("- Сформирован, epoch millis: `${export.generatedAtEpochMillis}`")
            appendLine("- Выбранный маршрут: `${export.transport.selectedRoute}`")
            appendLine("- Включённые режимы связи: `${export.transport.enabledTransportModes.joinToString()}`")
            appendLine("- Регистрация: `${export.transport.registrationState}`")
            appendLine("- Обнаружение: `${export.transport.discoveryState}`")
            appendLine("- Найдено устройств транспортом: `${export.transport.transportDiscoveredPeerCount}`")
            appendLine("- Префиксы отпечатков найденных устройств: `${export.transport.observedPeerFingerprintPrefixes.joinToString().ifBlank { "нет" }}`")
            appendLine("- Ручные устройства: `${export.transport.manualPeerCount}`")
            appendLine("- Видимые P2P-устройства: `${export.transport.p2pVisibleDeviceCount}`")
            appendLine("- Статус этого P2P-устройства: `${export.transport.p2pThisDeviceStatus ?: "нет"}`")
            appendLine("- Циклы обнаружения: `${export.transport.discoveryCycleCount}`")
            ...
            appendLine("- Отклонено P2P TXT: `${export.transport.p2pTxtRejectedCount}`")
            appendLine("- P2P TXT с привязанным peer: `${export.transport.p2pTxtBoundPeerCount}`")
            appendLine("- Видимые P2P-устройства без привязки: `${export.transport.p2pUnboundVisibleDeviceCount}`")
            appendLine("- Последняя ошибка binding Wi‑Fi Direct: `${export.transport.wifiDirectLastBindingError ?: "нет"}`")
            appendLine("- Локальный endpoint транспорта: `${export.transport.localAddresses.joinToString()}:${export.transport.localPort ?: "нет"}`")
            appendLine("- P2P interface addresses: `${export.transport.p2pInterfaceAddresses.joinToString().ifBlank { "нет" }}`")
            appendLine("- Группа Wi‑Fi Direct создана: `${export.transport.wifiDirectGroupFormed ?: "нет"}`")
            appendLine("- Это group owner Wi‑Fi Direct: `${export.transport.wifiDirectIsGroupOwner ?: "нет"}`")
            appendLine("- Роль в группе Wi‑Fi Direct: `${export.transport.wifiDirectGroupRole}`")
            appendLine("- Адрес group owner Wi‑Fi Direct: `${export.transport.wifiDirectGroupOwnerAddress ?: "нет"}`")
            appendLine("- Локальный P2P-адрес Wi‑Fi Direct: `${export.transport.wifiDirectLocalP2pAddress ?: "нет"}`")
            appendLine("- Адрес bind сервера Wi‑Fi Direct: `${export.transport.wifiDirectServerBindAddress ?: "нет"}`")
            appendLine("- Последний endpoint отправки Wi‑Fi Direct: `${export.transport.wifiDirectLastSendHost ?: "нет"}:${export.transport.wifiDirectLastSendPort ?: "нет"}`")
            appendLine("- Состояние endpoint binding Wi‑Fi Direct: `${export.transport.wifiDirectEndpointBindingState}`")
            appendLine("- Причина endpoint binding Wi‑Fi Direct: `${export.transport.wifiDirectEndpointBindingReason ?: "нет"}`")
            appendLine("- Префикс отпечатка relationship peer Wi‑Fi Direct: `${export.transport.wifiDirectRelationshipPeerFingerprintPrefix ?: "нет"}`")
            appendLine("- Последняя цель connect Wi‑Fi Direct: `${export.transport.wifiDirectLastConnectDeviceName ?: "нет"} / ${export.transport.wifiDirectLastConnectDeviceAddress ?: "нет"}`")
            appendLine("- Последний group owner intent Wi‑Fi Direct: `${export.transport.wifiDirectLastConnectGroupOwnerIntent ?: "нет"}`")
            appendLine("- Последний результат connect Wi‑Fi Direct: `${export.transport.wifiDirectLastConnectResult ?: "нет"}`")
            appendLine("- Последняя причина сбоя connect Wi‑Fi Direct: `${export.transport.wifiDirectLastConnectFailureReason ?: "нет"}`")
            appendLine("- Попытки connect Wi‑Fi Direct: `${export.transport.wifiDirectConnectAttempts.size}`")
            appendLine("- Найденные peer endpoints Wi‑Fi Direct: `${export.transport.wifiDirectDiscoveredPeers.size}`")
            appendLine("- Видимые устройства Wi‑Fi Direct: `${export.transport.wifiDirectVisibleDevices.size}`")
            appendLine("- Диагностика TXT records Wi‑Fi Direct: `${export.transport.wifiDirectTxtRecords.size}`")
            appendLine("- Привязанные endpoints Wi‑Fi Direct: `${export.transport.wifiDirectBoundEndpoints.size}`")
            appendLine("- Кандидаты ретрансляции реалма: `${export.transport.realmRelayCandidateCount}`")
            appendLine("- Префиксы кандидатов ретрансляции реалма: `${export.transport.realmRelayCandidates.map { it.peerFingerprintPrefix }.joinToString().ifBlank { "нет" }}`")
            export.transport.wifiDirectPermissions?.let { permissions ->
                ...
                appendLine("- Wi‑Fi Direct fine location declared/granted/app-op: `${permissions.fineLocationDeclared}/${permissions.fineLocationGranted}/${permissions.fineLocationAppOp ?: "нет"}`")
                appendLine("- Предупреждение разрешений Wi‑Fi Direct: `${permissions.warning ?: "нет"}`")
            }
            appendLine("- Последние попытки маршрута: `${export.transport.recentRouteAttempts.size}`")
            appendLine("- Размер очереди: `${export.queueSize}`")
            appendLine("- Последний статус пакета: `${export.lastPacketStatus}`")
            appendLine("- Версия приложения: `${export.appVersionName}`")
            appendLine("- Тип сборки: `${export.appBuildType}`")
            appendLine("- Git SHA: `${export.gitSha}`")
            appendLine("- Модель устройства: `${export.deviceModel}`")
            appendLine()
            appendLine("## Счётчики")
            appendLine()
            ...
            appendLine()
            appendLine("## Проверка Маршрута")
            appendLine()
            appendLine("- Режим проверки: `${export.debugSmoke.evidenceMode}`")
            ...
            appendLine()
            appendLine("- Режим целостности: `${export.integrityMode}`")
            appendLine()
            appendLine("`${export.claimBoundary}`")
            appendLine()
            appendLine("Граница evidence: защищённый путь данных сообщения, проверка допуска профиля, диагностика транспорта и двухтелефонные проверки.")
        }
